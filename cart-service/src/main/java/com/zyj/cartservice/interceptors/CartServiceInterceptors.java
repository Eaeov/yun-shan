package com.zyj.cartservice.interceptors;
...
public class CartServiceInterceptors implements HandlerInterceptor {
    ...
    private boolean validateGatewayKey(String gatewayKey, HttpServletResponse response) {
        if (StrUtil.isBlank(gatewayKey)) {
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            log.warn("购物车服务拦截：网关密钥为空");
            return false;
        }

        if (!gatewayConfig.getSecretKey().equals(gatewayKey)) {
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            log.warn("购物车服务拦截：网关密钥验证失败");
            return false;
        }
        return true;
    }
